...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Subtract 1 to ensure we don't miss the triggering message
    ...
    try {
      ...
      // Fall back: search for unread Amazon emails directly
      console.log('Falling back to search...');
      const searchRes = await gmail.users.messages.list({
        userId: process.env.GMAIL_ADDRESS,
        q: 'is:unread from:(auto-confirm@amazon.com OR return@amazon.com OR refund@amazon.com)',
        maxResults: 10,
      });
      messageIds = (searchRes.data.messages || []).map(m => m.id);
      console.log(`Found ${messageIds.length} messages via search fallback`);
    }
